...
Module connection
    ...
    Public Sub createConnection()
        ...
        constr = " Server=" & mdbserver & ";Database=" & mdbname & ";User Id=" & mdbuserid & "; Password=" & mdbpwd & "; MultipleActiveResultSets=True"
        'con.ConnectionString = " Server=" & mdbserver & ";Database=" & mdbname & ";User Id=" & mdbuserid & "; Password=" & mdbpwd & "; MultipleActiveResultSets=True"
        'con.Open()
        ...
        Try
            ...
            If String.IsNullOrWhiteSpace(con.ConnectionString) Then
            ...
            If con.State = ConnectionState.Open _
           OrElse con.State = ConnectionState.Connecting Then
                Exit Sub
            End If
    ...
    Public Sub checkConnection()
        ...
        Select Case con.State
            ...
            Case ConnectionState.Connecting
                Exit Sub   ' ✅ WAIT – do NOT Open again
    ...
    End Sub
    Public Function ExecuteTransactionWithCommands(cmds As List(Of SqlCommand)) As Boolean
        merr = ""
        merr2 = ""
        checkConnection()
    ...
    Public Function GetDataSet(ByVal spName As String, ByVal params As List(Of SqlParameter)) As DataSet
    ...
    '    New SqlParameter("@FromDate", SqlDbType.Date) With {.Value = fromDate},
    '    New SqlParameter("@ToDate", SqlDbType.Date) With {.Value = toDate},
    '    New SqlParameter("@BranchId", SqlDbType.Int) With {.Value = branchId}
    '}

    'Dim ds As DataSet = GetDataSet("INSGetGatepasstransport", params)

    'cmbto.DataSource = ds.Tables(0)
    'cmbto.DisplayMember = "TRANSPORT"


    Public Function ExecuteStoredProcedure(spName As String, params As List(Of SqlParameter)) As Boolean
        ...
        Using con As New SqlConnection(constr)
            ...
            Using cmd As New SqlCommand(spName, con, trans)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.CommandTimeout = 600
    ...
    ''    New SqlParameter("@FromDate", SqlDbType.Date) With {.Value = #2025-01-01#},
    ''    New SqlParameter("@ToDate", SqlDbType.Date) With {.Value = #2025-01-31#}
    ''}

    ''    Dim ok As Boolean = ExecuteStoredProcedure("INSGetGatepasstransport", p)
    ...
    Public Function GenerateNewIDNumber(ByVal srcTable As String, ByVal srcField As String) As Double
        ...
        dt = getDataTable("SELECT MAX(" & srcField & ") FROM " & srcTable)
