...
Public Class frmbarcode
    ...
    Private Sub loaddatan()
        If con.State = ConnectionState.Closed Then
            con.Open()
        End If
        Dim CMD As New SqlCommand("select stkcode,size,itcode,sizecode,mrprate,prate,qty FROM purchaseline WHERE purcno=" & Microsoft.VisualBasic.Val(txtbno.Text) & " and cmp_id='" & mcmpid & "'", con)
                            ...
                            If Len(RTrim(DR.Item("mtrstot"))) > 0 Then
                                ...
                                For i = 0 To UBound(mshmtr)
                                    ...
                                    If IsDBNull(DR.Item("MRP")) = False Then
                                        ...
                                        .set_TextMatrix(.Row, 5, Trim(Microsoft.VisualBasic.Format(Val(DR.Item("mrp")), "###########0.00")))
                                    Else
                                        .set_TextMatrix(.Row, 5, 0)
                                    End If
                                    If IsDBNull(DR.Item("rate")) = False Then
                                        '.set_TextMatrix(.Row, 6, DR.Item("rate"))
                                        .set_TextMatrix(.Row, 6, Trim(Microsoft.VisualBasic.Format(Val(DR.Item("rate")), "###########0.00")))
                                    Else
    ...
    Private Sub crtmptabst()
        ...
        If con.State = ConnectionState.Closed Then
            con.Open()
        End If
        Dim dCMD As New SqlCommand(msql, con)
    ...
    Private Sub crtmptab()
        ...
        Dim dCMD As New SqlCommand(msql4, con)
        'Dim CMD2 As New SqlClient.SqlCommand("DELETE FROM BPORD WHERE BNO=" & Val(txtbno.Text), con)
        ...
        Dim crCMD As New SqlCommand(msql, con)
        'Dim CMD2 As New SqlClient.SqlCommand("DELETE FROM BPORD WHERE BNO=" & Val(txtbno.Text), con)
        'If con.State = ConnectionState.Closed Then
        '    con.Open()
        'End If
    ...
    Private Sub cmdok_ClickButtonArea(ByVal Sender As System.Object, ByVal e As System.EventArgs)
        'Call loaddata()
    End Sub

    Private Sub cmdexit_ClickButtonArea(ByVal Sender As System.Object, ByVal e As System.EventArgs)
        Me.Close()
    End Sub

    Private Sub cmdprint_ClickButtonArea(ByVal Sender As System.Object, ByVal e As System.EventArgs)
        'Call barprn()
    ...
    Private Sub loadno()
        ...
        Else
            ...
            msql = "select b.DocNum,b.DocDate, b.pIndicator,DATEPART(mm,b.DocDate) as mkmon,DATEPART(yyyy,b.docdate) as yr from " & Trim(mfildet) & " b with (nolock) left join OFPR p on p.Indicator=b.PIndicator where b.DocNum=" & Val(txtbno.Text) & " and b.pindicator='" & Trim(cmbyear.Text) & "'"
    ...
    Private Sub barprnsap1()
        ...
        For i = 1 To flx.Rows - 1
            ...
            If Len(Trim(flx.get_TextMatrix(i, 0))) > 0 Then
                ...
                If DR1 IsNot Nothing AndAlso Not DR1.IsClosed Then
                    DR1.Close()
                End If
                ...
                Try
                    ...
                    If DR1.HasRows = True Then
                        While DR1.Read
                            ...
                            If Len(Trim(DR1.Item("ldata"))) > 0 Then
                                Dim DR4 As SqlDataReader = Nothing

                                'If DR4 IsNot Nothing AndAlso Not DR4.IsClosed Then
                                '    DR4.Close()
                                'End If

                                'Dim cmd4 As New SqlCommand(msql, con)
                                If con.State = ConnectionState.Closed Then
                                    con.Open()
                                End If
                                Dim cmd4 As New SqlCommand(msql, con)
                ...
                If DR2 IsNot Nothing AndAlso Not DR2.IsClosed Then
                    DR2.Close()
                End If
                'Dim CMD2 As New SqlCommand(msql2, con)
                If con.State = ConnectionState.Closed Then
                    ...
                    If DR2.HasRows = True Then
                        While DR2.Read
                            ...
                            If Len(Trim(DR2.Item("ldata"))) > 0 Then
                                ...
                                Dim DR5 As SqlDataReader = Nothing

                                If DR5 IsNot Nothing AndAlso Not DR5.IsClosed Then
                                    DR5.Close()
                                End If

                                'Dim cmd5 As New SqlCommand(msql, con)
                                If con.State = ConnectionState.Closed Then
                                    con.Open()
                                End If
                                Dim cmd5 As New SqlCommand(msql, con)
        ...
        Else
            ...
            Dim filePathname As String = mlinpath & "sbarcodE.txt"
    ...
    End Sub
    Private Sub updateprn()

        'msql = "SELECT isnull(u_stikprinted,'') u_stikprinted FROM oinv where docnum=" & Val(txtbno.Text) & " and month(docdate)=" & Val(cmbmont.Text) & " and year(docdate)=" & Val(cmbyr.Text)
        ...
        If Trim(cmbprnon.Text) = "SALES" Then
            executeQuery("update oinv set u_stikprinted='Y',u_stikprndate=getdate() where  (isnull(u_stikprinted,'')='' or isnull(u_stikprinted,'')='N') and docnum=" & Val(txtbno.Text) & " and month(docdate)=" & Val(cmbmont.Text) & " and year(docdate)=" & Val(cmbyr.Text))
        ElseIf Trim(cmbprnon.Text) = "DATE ORDER" Then
            'mfil = "DLN1"
            'mfildet = "ODLN"
            executeQuery("update odln set u_stikprinted='Y',u_stikprndate=getdate() where  (isnull(u_stikprinted,'')='' or isnull(u_stikprinted,'')='N') and docnum=" & Val(txtbno.Text) & " and month(docdate)=" & Val(cmbmont.Text) & " and year(docdate)=" & Val(cmbyr.Text))
        ...
        'If DR.HasRows = True Then
        '    While DR.Read
        '        If DR.Item("u_stikprinted") = "N" Then
        '            PrintLine(1, TAB(0), DR.Item("firstdet"))
        '    End While
           ...
           & "CASE when CHARINDEX('CMS',k.u_length)<=0  and cast(k.u_length as real)>0 and (rtrim(upper(k.u_itemgrp)) in( select u_itemgrp from [@ins_oplm] where CHARINDEX('CMS',u_size)<=0 group by u_itemgrp) OR CHARINDEX('DHOTI',UPPER(K.U_ItemGrp))>0) then  RTRIM(k.u_width)+'cmX'+rtrim(lTRIM(cast(cast(k.u_length as real)*100 as char(10)))) +'cm' else  replace(RTRIM(k.u_length),'S','X')+ rtrim(cast(convert(decimal(16,2),t1.quantity) as char(10)))+'M' end as msize2,'MFD ' + substring(datename(m,GETDATE()),1,3) +' '+ CAST(DATEPART(YYYY,GETDATE()) as CHAR(4)) as mfd, " & vbCrLf _
            ...
            & " where t0.docnum=" & Val(txtbno.Text) & " and t1.TreeType<>'I' and t1.u_catalogname='" & Trim(flx.get_TextMatrix(i, 1)) & "' and  k.u_size='" & Trim(flx.get_TextMatrix(i, 13)) & "'" & vbCrLf
                    ...
                    If mksno = Val(txtstickcol.Text) Then
                        ...
                        msql = "SELECT  [docentry],[date],[linenum],[headtype],[firstdet],[lrow],[lcol],[fontdet],[ldata],[secdet] FROM bardet where docentry=" & Val(txtdocno.Text) & " and headtype='H' order by docentry,linenum"
                        'Header
                        Dim CMDh As New SqlCommand(msql, con)
                        If con.State = ConnectionState.Closed Then
                            con.Open()
                        End If
    ...
    Private Sub barprnsapmulti2()
        ...
        If mksno2 = 1 Then
            msql2 = "SELECT  [docentry],[date],[linenum],[headtype],[firstdet],[lrow],[lcol],[fontdet],[ldata],[secdet] FROM bardet where docentry=" & Val(txtdocno.Text) & " and headtype='F' order by docentry,linenum"
            Dim CMD6 As New SqlCommand(msql2, con)
            If con.State = ConnectionState.Closed Then
                con.Open()
            End If
    ...
    Private Sub CButton1_ClickButtonArea(ByVal Sender As System.Object, ByVal e As System.EventArgs)
